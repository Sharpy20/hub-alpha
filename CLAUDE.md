...
| 211 | [ ] | **Still queued:** **AWOL/Missing Patients guide** - BLOCKED: source `Absent_and_Missing_Patients...doc` is old binary .doc and won't extract; Mike to re-save as .docx. Home bookmark-wheel resize for 15in laptops (needs browser verify). The 86 placeholder referral-form links (link-to-source - needs per-form verification). `links task.docx` -> SystmOne FOCUS guide URLs (FOCUS login-gated). Deepen Section 136 from its PDFs (OnEPR S136 assessment, Right Care Right Person, leaflets). See memory `session-28-*`. |
